...
def mask_generator(shadow, shadow_free):
    ...
    for i in range(shadow.shape[0]):
        im_f = to_gray(to_pil(((shadow_free[i:i+1].data.squeeze(0) + 1.0) * 0.5).cpu())) #shadow_free--[-1,1]
        im_s = to_gray(to_pil(((shadow[i:i+1].data.squeeze(0) + 1.0) * 0.5).cpu()))   #shadow  --[-1,1]
...
def weights_init_normal(m):
    ...
    if classname.find('Conv') != -1:
        torch.nn.init.normal_(m.weight.data, 0.0, 0.02) #用正态分布的值填充，为什么卷积层只需要初始化w，batchnorm层需要全都初始化呢
    elif classname.find('BatchNorm2d')!=-1 or classname.find('BatchNorm')!= -1:
        torch.nn.init.normal_(m.weight.data, 1.0, 0.02)
        torch.nn.init.constant_(m.bias.data, 0.0)    #constant是用值进行填充。这里是用0填充bias值
